...
object OrderResponse {
    ...
    fun orderDetailSuccess(orderInfo: OrderInfo) = ResponseEntity.ok(orderInfo)

    fun myOrderSuccess(orderInfo: List<OrderInfo>) = ResponseEntity.ok(orderInfo)

    fun shopOrderListSuccess(orderInfo: List<OrderInfo>) = ResponseEntity.ok(orderInfo)
    ...
    fun deliveryCompletedSuccess() = ResponseEntity.ok(DELIVERY_COMPLETED_SUCCESS)

    fun orderCancelSuccess() = ResponseEntity.ok(ORDER_CANCEL_SUCCESS)
}